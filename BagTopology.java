...
public class BagTopology implements Serializable{

	private static final Logger LOG = LoggerFactory.getLogger(BagTopology.class);
	private static Bagging bag;
	...
	public static class TrainBolt extends BaseBasicBolt {
		...
		public void execute(Tuple tuple, BasicOutputCollector collector) {
			...
			bag.loadData(str);
			System.out.println("DATA REC "+bag.wholedata.size()+": "+str);
			if(bag.wholedata.size() % 600 == 0){				
				System.out.println("finish collecting training data. data record read "+ bag.wholedata.size());
				bag.split(80); //split the data when finishing collecting
				bag.replacelabel(number); //unify the label
				bag.set_boundary(); //parse the data to get feature information
				System.out.println("training regression model . . . done!");
				bag.train(4);  //start to train the model with 4 trees
				List<Boolean> prediction = bag.predict();
				List<Boolean> actual = bag.actual(bag.testing);
				System.out.println("The Accuracy is ...: "+bag.accuracy(actual,prediction)*100); //print out the accuracy
				System.out.println(bag.training.size() + "+" +bag.testing.size());				
			}
		...
		public void execute(Tuple input, BasicOutputCollector collector) {
			...
			if(value.length == number - 1){
				if(bag.rf == null){
					...
					bag.set_boundary(); //parse the data to get feature information
					System.out.println("training regression model . . . done!");
					bag.train(4);  //start to train the model
					List<Boolean> prediction = bag.predict();
					List<Boolean> actual = bag.actual(bag.testing);
					System.out.println("The Accuracy is ...: "+bag.accuracy(actual,prediction)*100); //print out the accuracy
					System.out.println(bag.training.size() + "+" +bag.testing.size());					
				...
				boolean label = bag.predict(record);
				String ll = label?bag.actualLabel[0]:bag.actualLabel[1]; // take the majority vote
